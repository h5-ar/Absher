        ...
        <tbody>
            ...
            <tr data-reservation-id="{{ $reservation->id }}">
                ...
                <td class="text-nowrap w-30 text-capitalize fs-5 fw-bolder text-center">
                    <x-Button.delete route="{{ route('SAreservation.delete', $reservation->id) }}" />
...
<script>
    $(document).ready(function() {
        ...
        function loadPassengers(reservationId, userName) {
            ...
            $.ajax({
                url: "{{ route('SAreservation.passengers') }}",
                method: 'GET',
                data: {
                    ...
                    if (response.success && response.passengers && response.passengers.length > 0) {
                        ...
                        response.passengers.forEach(passenger => {
                            ...
                            <tr>
                                ...
                                <td class="text-nowrap w-30 text-capitalize fs-5 fw-bolder text-center">
                                   <x-Button.edit route="{{ route('SApassenger.edit', '') }}/${passenger.id}" />
                                   <x-Button.delete route="/SuperAdmin/passenger/delete/${passenger.id}/${passenger.reservation_id}" />
                            </tr>
                        `;
                        });
