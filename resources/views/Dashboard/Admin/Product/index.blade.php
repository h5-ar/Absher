                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row ">
                            ...
                            @if (request()->routeIs('dashboard.products.index'))
                            ...
                            @endif
                            <div class="col-xl-2 col-lg-2 col-md-2 col-sm-12 col-12">
                                <x-inputs.Multi-Vertical.select selectId="searchInStock" lable="Quantity"
                                    title="Filter By Status" size="col-12" onchange="onSearchInStock(event)">
                                    <x-inputs.option lable="all" value="all" selected />
                                    <x-inputs.option lable="In Stock" value="inStock" />
                                    <x-inputs.option lable="Out Of Stock" value="outOfStock" />
                                </x-inputs.Multi-Vertical.select>
                            </div>
                            <div class="col-xl-2 col-lg-2 col-md-2 col-sm-12 col-12">
                            ...
                            <div class="col-xl-2 col-lg-2 col-md-2 col-sm-12 col-12">
                                <x-inputs.Multi-Vertical.select title="Categories" selectId="search_category_id"
                                    lable="Categories" size="col-12" onchange="onSearchCategory(event)">
                                    <x-inputs.option lable="all" value="all" selected />
                    ...
                    <div id="page-data">
                        @include('Dashboard.Admin.Product.sections.indexTable', ['products' => $products])
                    </div>
                </div>
            </div>
        </div>
...
@push('layout-scripts')
    <script>
        ...
        function updateArchive(productId) {
            $(`#archived-${productId}`).submit();
        }

        function onSearchInStock(event) {
            event.preventDefault();
            const urlParams = new URLSearchParams(window.location.search);
            urlParams.set('quantity', event.target.value);
            urlParams.set('search', 1);
            urlParams.set('page', 1);
            const url = window.location.href.split('?')[0];
            const fullurl = `${url}?${urlParams}`;
